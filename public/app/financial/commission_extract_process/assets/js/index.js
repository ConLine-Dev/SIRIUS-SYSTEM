...
// Função para calcular o valor total da coluna commission considerando apenas os itens visíveis da tabela
function calcularValorTotal() {
    let valorTotal = 0;
    table['tableExtractComissionProcess'].rows({ search: 'applied' }).data().each(function (row) {
        valorTotal += parseFloat(row.commission);
    });


    const numeroRegistrosFiltrados = table['tableExtractComissionProcess'].rows({ search: 'applied' }).count();
    document.querySelector('.total-processes').textContent = numeroRegistrosFiltrados;
...
function filtrarPorData() {
    if (table['tableExtractComissionProcess']) {
        const dataTable = table['tableExtractComissionProcess'];

        // Mostra o loading quando iniciar o filtro
        ...
        dataTable.ajax.reload(function() {
            ...
            if (dataDe && dataAte) {
                ...
                if (!isNaN(dataDeFormatada.getTime()) && !isNaN(dataAteFormatada.getTime())) {
                    ...
                    filtros.push(row => {
                        const dataStr = row['create_date']; // Substitua pelo índice correto da coluna de datas
            ...
            if (peopleValue !== "0") {
                filtros.push(row => {
                    const commissionedType = row['commissioned_type'];
                    const idSeller = row['id_seller'];
                    const idInside = row['id_inside'];
                    
                    // Verifica se o tipo de comissionado corresponde ao ID correto
                    if (commissionedType == 1) { // Vendedor
                        return idSeller == peopleValue;
                    } else if (commissionedType == 2) { // Inside Sales
                        return idInside == peopleValue;
                    }
                    return false;
                });
            } else {
            ...
            // Remove o loading após aplicar os filtros
            document.querySelector('#loader2').classList.add('d-none');
        }, false); // 'false' impede a ressincronização completa dos dados, mantendo o estado atual
    } else {
        console.error("'tableExtractComissionProcess' não está definido na variável 'table'.");
    }
...
async function tableFinancialExpenses() {
    ...
    if ($.fn.DataTable.isDataTable('#tableExtractComissionProcess')) {
        $('#tableExtractComissionProcess').DataTable().destroy();
    }

    // Criar a nova tabela com os dados da API
    table['tableExtractComissionProcess'] = $('#tableExtractComissionProcess').DataTable({
        dom: 'Bfrtip',
        order: [[0, 'desc']],
        ...
        ajax: {
            url: `/api/headcargo/commission/listRegisterProcess`,
            type: 'POST',
            dataSrc: '',
            complete: function() {
                // Remove o loading quando a tabela terminar de carregar
                document.querySelector('#loader2').classList.add('d-none');
                calcularValorTotal();
            }
        },
        columns: [
            { 
                data: 'commission_reference', 
                render: function(data, type, row) {
                    return data;
                }
            },
            { 
                data: 'reference_process', 
                render: function(data, type, row) {
                    return data;
                }
            },
...
async function initializeDatePicker() {
    ...
    const startOfMonth = new Date(today.getFullYear(), today.getMonth(), 1); // 1° do mês atual

    flatpickr("#inputDateCreate", {
        mode: "range",
        dateFormat: "d M Y",
        defaultDate: [startOfMonth, today], // Define o intervalo de data do início do mês até hoje
    });
...
async function eventClick() {
    ...
    $('#tableExtractComissionProcess tbody').on('click', 'tr', function () {
        if ($(this).hasClass('selected')) {
            $(this).removeClass('selected');
        }
        else {
            table['tableExtractComissionProcess'].$('tr.selected').removeClass('selected');
