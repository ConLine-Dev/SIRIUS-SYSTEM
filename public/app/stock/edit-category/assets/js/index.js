...
async function getCategoryInfo(id) {
   const categoryInfo = await makeRequest(`/api/product/getCategoryById`, 'POST', { id: id });

   document.getElementById('input-category-name').value = categoryInfo[0].Category;
   document.getElementById('textarea-observation').value = categoryInfo[0].observation;
   await setSelectDepartmentFromDB(categoryInfo[0].IdDepartment)
}
...
async function setSelectDepartmentFromDB(value) {
   selectDepartments.setChoiceByValue(value);
};
...
document.getElementById('btn-save').addEventListener('click', async function(e) {
   ...
   const idCategory = await getUrlId();  
   ...
   if (inputsValid && selectsValid) {
      const updateCategory = await makeRequest(`/api/product/updateCategory`, 'POST', { departmentId: selectDepartmentstValue, name: inputCategoryName, textareaObservation, idCategory });
      window.close();
   }
