...
// Função para buscar os dados do desconto e preenche os campos do formulário com as informações retornadas.
async function getViewDiscount(id) {
    // Faz uma requisição à API para obter os dados do desconto
    const discount = await makeRequest(`/api/rh-payroll/getView`, 'POST', { id: id });

    // Preenche os campos do formulário com os dados retornados
    document.querySelector('select[name="responsible"]').value = discount.responsible;
    document.querySelector('input[name="monthYear"]').value = discount.monthYear;
    document.querySelector('input[name="effectiveDate"]').value = discount.effectiveDate;
    document.querySelector('select[name="discount"]').value = discount.discount;
    document.querySelector('textarea[name="observation"]').value = discount.observation;

    // Atualiza a seleção do elemento ChoicesJS para o campo "responsible"
    sAllResponsible.setChoiceByValue(discount.responsible.toString());
};
...
async function getDiscount() {
    ...
    const result = await makeRequest(`/api/rh-payroll/update`, 'POST', form);
    window.close();
};
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const discount_id = await getDiscountInfo()
