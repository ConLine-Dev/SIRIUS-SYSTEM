...
async function fillFormWithTestData() {
    // Seleciona todos os elementos input, select e textarea que possuem um atributo 'name'
    const elements = document.querySelectorAll('form input[name],form textarea[name],form select[name]');
    
    // Dados de teste para os campos
    const testData = {
        name: 'João',
        family_name: 'Silva',
        ...
        workload: '40',
        emailBusiness: 'joao.silva@empresa.com',
        cpf: '12345678909',
        rg: '123456789',
        languages: ['Inglês', 'Espanhol'], // Exemplo de seleção de línguas
        photo: null, // Você pode adicionar um arquivo para teste se necessário
        documentsData: [
            { name: 'RG', date: '2021-06-01', file: null },
            { name: 'CPF', date: '2020-12-10', file: null }
        ],
        bankingInformation: [
            { bank: 'Banco XYZ', account: '12345-6', agency: '7890' }
        ],
        certifications: [
            { qualification: 'Bacharelado em TI', institution: 'Universidade ABC', completionDate: '2015-12-15', file: null }
        ]
    };
    ...
    // Simula a seleção de idiomas (se estiver usando ChoicesJS ou outra biblioteca)
    sAllLanguages.setValue(testData.languages);

    // Preenche os documentos e certificações
    Documents = testData.documentsData;
    BankingInformation = testData.bankingInformation;
    Certifications = testData.certifications;

    // Exemplo para adicionar uma imagem de teste (caso você queira adicionar uma)
    const photoInput = document.querySelector('input[name="photo"]');
    if (photoInput && testData.photo) {
        const file = new File([new Blob(['test'])], 'photo.jpg', { type: 'image/jpeg' });
