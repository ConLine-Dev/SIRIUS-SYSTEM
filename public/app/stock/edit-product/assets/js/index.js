...
// Função para inserir nos campos o resultado do banco de dados
async function getProductInfo(id) {
   const productInfo = await makeRequest(`/api/product/getProductById`, 'POST', { id: id });

   document.getElementById('input-product-name').value = productInfo[0].Product;
   document.getElementById('input-ncm').value = productInfo[0].ncm;
   document.getElementById('textarea-observation').value = productInfo[0].observation;
   await setSelectCategoryFromDB(productInfo[0].IdCategory)
}
...
// Função para selecionar a categoria no SELECT
async function setSelectCategoryFromDB(value) {
   selectCategory.setChoiceByValue(value);
};
...
document.getElementById('btn-save').addEventListener('click', async function(e) {
   ...
   const idProduct = await getUrlId();   
   ...
   if (inputsValid && selectsValid) {
      const updateProduct = await makeRequest(`/api/product/updateProduct`, 'POST', { name: inputProductName, ncm: inputNCM, categoryId: selectCategoryValue, textareaObservation, idProduct});
      window.close();
   }
