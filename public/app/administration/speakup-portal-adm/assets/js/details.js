...
async function saveOccurrence() {
  ...
  await makeRequest(`/api/speakup-portal/updateStatus`, 'POST', {id, status});
...
async function createHistory() {
  ...
  for (let index = getComments.length - 1; index >= 0; index--) {
    ...
    let commentName = '';
    if (getComments[index].screen == 'adm') {
      commentName = 'Responsável';
    } else {
      commentName = 'Denunciante';
    }
...
async function createSelect() {
  ...
  const status = await makeRequest(`/api/speakup-portal/getStatus`);

  for (let index = 0; index < status.length; index++) {
    const option = document.createElement("option");
    option.value = status[index].id;
    option.text = status[index].description;
...
async function printOcurrenceDetails() {
  ...
  document.getElementById("status").value = getDetails[0].id_status;
