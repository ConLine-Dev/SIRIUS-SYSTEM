(function(){
  ...
  function renderTemplates() {
    ['responsibilities', 'requirements', 'benefits', 'niceToHave'].forEach(type => {
      ...
      if (container) {
        ...
        // Renderizar apenas os itens existentes, permitindo seções vazias
        if (items.length > 0) {
          items.forEach(item => {
  ...
  window.removeTemplateItem = function(button) {
    ...
    // Permitir remover todos os itens, incluindo o último
    itemDiv.remove();
  };
  ...
  async function saveGlobalTemplates() {
    ...
    try {
      // Mostrar loading com mensagem específica para templates globais
      Swal.fire({
        title: 'Salvando Template Global...',
        text: 'Aplicando templates para todos os departamentos. Isso pode levar alguns segundos.',
      ...
      if (response.ok) {
        ...
        Swal.fire({
          icon: 'success',
          title: 'Template Global Salvo!',
          text: 'Templates aplicados com sucesso para todos os departamentos',
          timer: 3000,
          showConfirmButton: false
        });
      } else {
        const errorData = await response.json().catch(() => ({ message: 'Erro desconhecido' }));
        throw new Error(errorData.message || 'Erro ao salvar template global');
      }
    } catch (error) {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Erro ao Salvar',
        text: error.message || 'Não foi possível salvar o template global'
      });
    }
  }
